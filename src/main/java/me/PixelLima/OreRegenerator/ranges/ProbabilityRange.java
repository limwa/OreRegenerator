 ...
 */
package me.PixelLima.OreRegenerator.ranges;

public class ProbabilityRange {
	
	private final float minimum, maximum;

	public ProbabilityRange(float minimum, float maximum) {
		if(minimum < 0F || maximum > 1F)
			throw new IllegalArgumentException("Minimum/Maximum values exceed probability limits");
		
		this.minimum = minimum;
		this.maximum = maximum;
	}
	
	public boolean isInRange(float probability) {
	...
	public boolean equals(Object obj) {
		...
		ProbabilityRange range = (ProbabilityRange) obj;
		return range.maximum == this.maximum && range.minimum == this.minimum;
	}
}
