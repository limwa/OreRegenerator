...
public class RegenerationTask implements Runnable {
	...
	private Material regen;
	
	public RegenerationTask(int min, int max, Block block) {
		this.block = block;
		
		float prob = r.nextFloat();
		
		if(!OreRegenerator.probabilities.forEach((range) -> {
			if(range.isInRange(prob)) {
				this.regen = range.getMaterial(block.getType());
				return false;
			}
			return true;
		}))
			taskId = Bukkit.getScheduler().scheduleSyncDelayedTask(OreRegenerator.plugin, this, min + r.nextInt(max + 1));
		else {
			taskId = -1;
			synchronized (OreRegenerator.scheduledBlocks) {
				OreRegenerator.scheduledBlocks.remove(block.getLocation());
			}
		}
	}
	
	public RegenerationTask(int min, int max, Block block, Material regenerate) {
		this.block = block;
		if(regenerate != null) {
			this.regen = regenerate;
			taskId = Bukkit.getScheduler().scheduleSyncDelayedTask(OreRegenerator.plugin, this, min + r.nextInt(max + 1));
		} else {
			taskId = -1;
			synchronized (OreRegenerator.scheduledBlocks) {
	...
	public void run() {
		...
		new RegenerationTask(OreRegenerator.OTHER_REGENS_MIN, OreRegenerator.OTHER_REGENS_MAX, block, regen);
	}
